        ...
        if os.path.exists(label_path):
            with open(label_path, "r") as label_file:
                for label in label_file:
                    ...
                    labels.append(int(1)) # Change labels to 1 (labeled 0 in dataset) because labeling zero makes no fucking sense
...
transform = A.Compose(
    [
        #A.ChannelDropout(p=0.2), # Thought is that this will generalize better to shifting wheater conditiotion -> Varying colors
        #A.Defocus(p=0.1), # Snow on the lens or poles that are to far/close to be in the focus of the camera
        #A.GaussNoise(p=0.5), # Some of the same effects as dropout layers in the network
        #A.Illumination(p=0.5), # Shifting light intensity, usual when driving in and out of a forest, snow, etc
        #A.RandomBrightnessContrast(p=0.5), # The camera exposure might be of
        A.HorizontalFlip(p=0.5), # To increase sample size
        A.VerticalFlip(p=0.5), # To increase sample size
